...
| `.claude/commands/*.md` (12) | `<repo>/.claude/commands/` | **MERGE** | Add any command you don't have (`matrix-hunt`, `radar-outreach`, `apply`, `rank`, `interview`, `power-story`, `outcome`, `expand`, `setup`, `add-portal`, `add-template`, `reset`). If you already have a personalized one, keep yours. |
