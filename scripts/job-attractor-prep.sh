...
GUARD='UNATTENDED PREP TASK — no human is present and you have NO shell/Bash. HARD RULES: you MUST NOT attempt to send, email, contact, connect, apply, or open anything (you have no way to, keep it that way). You ONLY research (WebSearch/WebFetch/Read/Grep/Glob) and WRITE review-ready drafts into documents/outreach-queue.md (STATUS: NEW) plus a one-line summary in documents/outreach-metrics.md. NEVER edit outreach_log.md, job_search_tracker.csv, WORKFLOW-RULES.md, CLAUDE.md, the ~/.claude memory store, or any file other than those two. Respect documents/blocked-employers-list.md and every hard filter. If unsure, write a NOTE in the queue rather than acting. You review everything when he returns.'
