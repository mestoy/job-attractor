...
description: >
  ...
  scrape jobs, /scrape
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(bun --version), Bash(bun run .agents/skills/*/cli/src/cli.ts *), WebFetch, WebSearch, Agent, AskUserQuestion, mcp__claude-in-chrome__tabs_context_mcp, mcp__claude-in-chrome__tabs_create_mcp, mcp__claude-in-chrome__navigate, mcp__claude-in-chrome__get_page_text, mcp__claude-in-chrome__read_page
---
...
**Use the installed CLI tools as the primary search mechanism** (LinkedIn, Built In, Welcome to the Jungle, Hiring Cafe). Also cover the Cloudflare-walled sites (Indeed, Glassdoor, Wellfound) on every scrape — **browser-assisted when Chrome is connected** (Step 1c), otherwise via WebSearch (Step 1d). Fall back to `WebSearch` for a CLI portal only if its CLI is unavailable or fails.
...
#### 1c. Browser-assisted sources (Indeed, Glassdoor, Wellfound — when Chrome is connected)

These three are Cloudflare/CAPTCHA-walled and have no CLI. If the **Claude-in-Chrome** browser tools are available and connected (a `tabs_context_mcp` call succeeds), read them from the user's real browser — see `browser-sources.md` (this directory) for the full workflow, URL patterns, and the **mandatory CAPTCHA/login rule** (the user solves challenges, never you). This is the richer path. If the browser is **not** connected, skip to 1d for these sites — do not retry the browser in a loop.

#### 1d. WebSearch sources (always available; CLI + browser fallback)

Use `WebSearch` for:
- **Indeed, Glassdoor, Wellfound** whenever the browser (1c) is not connected — they still belong in every scrape. Use the `site:` query recipes in `search-queries.md`.
- Any other portal listed in `search-queries.md` without a corresponding directory under `.agents/skills/`
- Any CLI portal whose CLI fails at runtime, or when bun is unavailable (Step 1a failed)

Use the site-specific query strings from `search-queries.md` directly as WebSearch queries. For a promising hit, try `WebFetch` on the posting URL for the description; if it is Cloudflare-blocked, keep the WebSearch title/snippet (company + role + location) and note that the description needs the URL opened.
