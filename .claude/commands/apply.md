...
> **Template note:** the specific LaTeX mechanics mentioned below (compile engine, page count, package names, cover-letter class) are illustrative of one particular template. **Always defer to your active template's manifest** — `05-cv-templates.md` and the template's `TEMPLATE.md` — for the real compile command, engine, page limit, and structure. For example, the bundled `plain-professional` template compiles with `pdflatex` to **1 page**, not `lualatex` to 2. Substitute your template's actual mechanics wherever this file says otherwise.
...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, and **language** of the posting (default English for US postings; note if it's in another language).
...
Using the framework from `04-job-evaluation.md`, evaluate the job posting against the candidate's profile. If you have wired an optional salary-lookup tool (not shipped with the kit), run:
...
- Always in **English**
- Follow the active template's format from `05-cv-templates.md` (the bundled `plain-professional`, unless you registered another via `/add-template`)
- Tailor the profile statement and experience bullets to the specific role
- Reframe skills and achievements to match job requirements
- Keep to **1 page** (the bundled `plain-professional` template; enforced by `scripts/verify_resume.py`)

### Cover Letter — CONDITIONAL, not automatic
⛔ Standalone letters are RETIRED. Decide by the form:
- The form **HAS free-response fields** → answer those. **No letter.**
- The form has **NO free-response fields** → build a tailored letter **matching the résumé's own template and font**, and append it **AFTER the résumé as one combined PDF** (`pdfunite`). Track which applications got one.
- **Match the language of the job posting** (US postings are in English -> English letter; if a posting is in another language, match it)
...
> ⚠️ CORRECTED. This block used to prescribe `lualatex` + `xelatex`/`cover.cls`/Raleway and
> "exactly 2 pages" — the RETIRED moderncv/cover.cls path, against assets this kit does not ship
> (there is no `cover.cls`, no moderncv class and no Raleway font anywhere in it). A résumé built
> from it fails `verify_resume.py`'s 1-page tripwire and contradicts this file's own Step 2 note.
> The governing doc is `documents/resume-build-checklist.md`; this now matches it.

```bash
cd cv && pdflatex -interaction=nonstopmode main_<company>.tex
# conditional appended letter (ONLY when the form has NO free-response fields):
# compile it with pdflatex too (résumé template, same font), then merge résumé-FIRST:
#   pdfunite main_<company>.pdf cover_<company>.pdf "<Your Name> - Resume + Cover - <Company>.pdf"
```

- CV compiles with **pdflatex** (the active 1-page `plain-professional` template). ⛔ The old lualatex + fontawesome5 note is RETIRED — it applied only to the moderncv/banking template.
- ⛔ RETIRED: xelatex / `cover.cls` / Raleway / fontspec. A conditional letter uses the résumé's own template and is `pdfunite`d résumé-first into one PDF. There is no standalone-letter compile path anymore.
...
- [ ] **Exactly 1 page** (the bundled `plain-professional` template; enforced by `scripts/verify_resume.py`)
- [ ] No orphaned entry titles — a job/education title line must never sit alone at the bottom of a page with its bullets on the next. This is the most common failure.
- [ ] Section headings are not isolated at a page top with only 1-2 lines below
- [ ] No awkward whitespace gaps

**Conditional appended letter (only if the form had NO free-response fields):**
- [ ] Letter is **1 page** and uses the résumé's own template and font
- [ ] Signature block visible, not cut off or pushed to another page
- [ ] Merged résumé-FIRST into a single PDF via `pdfunite`
...
- **Appended letter uses the wrong font or breaks compile:** it must use the RÉSUMÉ's template and font, not a separate letter class. ⛔ The old `\fontspec`/Raleway workaround is RETIRED — it belonged to `cover.cls`, which this kit does not ship.
...
- [ ] **Reading order matches the visual order** — section headings appear in the same sequence as on the page, and lines from different sections are not interleaved. The bundled `plain-professional` template is single-column and safe; custom templates registered via `/add-template` with sidebars or multi-column layouts are where this breaks.
...
**3. Keyword coverage.** Reuse the required/preferred keyword list you extracted in Step 1 — do not re-derive it. Match each keyword against the extracted text, **in the posting's language** (US postings are in English; for a posting in another language, match its keywords in that language — where the CV legitimately covers the concept in English, count it as synonym-only and note the language difference). Report a table:
...
Run the full verification checklist from `documents/resume-build-checklist.md` now — this is the **only** verification pass in the workflow. Re-read it once here to verify final state on disk matches your mental model after the Step 4 and Step 5 edits.

### Verification Checklist
Report pass/fail for each item in the `documents/resume-build-checklist.md` verification checklist (factual accuracy, targeting, consistency, quality).
