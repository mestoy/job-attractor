...
Then proceed to Step 3 to write `documents/PROFILE.md` (the primary profile) and populate the other non-skill files (`cv/main_example.tex`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps. ⛔ Step 3 never edits `CLAUDE.md` — it is a lean index.
...
- **Job portals:** "The framework includes US job-search tools for LinkedIn, Built In, and Welcome to the Jungle. Are these the right ones for you, or do you use other sites (e.g. Wellfound, Dice, Indeed, or specific company career pages)?" Note: additional sites can be added with `/add-portal`, and any site without a CLI is still reachable via the `/scrape` WebSearch fallback.
...
Once data collection is complete, generate or finish populating the following files. **`documents/PROFILE.md` is the primary profile output — the single source of truth the assistant loads on demand.** **For Path A**, the seven skill files are already populated by Step A7; check each before writing and skip if its content is no longer placeholder text.

### 1. Write `documents/PROFILE.md` (the primary profile — every path does this)
Map the collected information onto PROFILE.md's sections, replacing every `[...]` placeholder: Identity; What you offer (2–3 quantified proofs, no "years of experience"); Your thesis/hook; Education; Professional Experience (reverse-chronological, with honest tenure-scoped metrics); Technical/Domain Skills; Certifications; Behavioral Profile; What excites you; Target sectors; Honesty guardrails (retire any figure the user can't source); Deal-breakers/hard filters (recorded as GATES, not preferences); Preferences; Company size. Leave no `[...]` token behind. This is the file `CLAUDE.md` and the pipeline skills point at, so it must be complete before the pipeline is usable.

⛔ **Do NOT edit `CLAUDE.md`.** It is a lean index that never holds profile data — editing it would re-inflate it and break clean `git pull` updates. Every profile fact lives in `documents/PROFILE.md`.
...
## Step 3.5: Write `scripts/kit_config.py` — the user must NEVER edit this by hand

**This step is what makes the kit work out of the box.** Every gate reads identity and filters from
`kit_config.py`. If it still holds placeholders, the résumé and outreach linters reject the user's
own correct work for reasons that look like bugs, and the screening filters silently apply a
stranger's deal-breakers. You already collected everything needed above; write it now.

Read `scripts/kit_config.py` first, then edit **in place**, preserving its comments and structure.

### 1. Identity (from the data you collected)
Replace the placeholder defaults inside `_env(...)`:
`OWNER_NAME` · `OWNER_SITE` (no scheme, e.g. `janedoe.com`) · `OWNER_EMAIL` · `OWNER_PHONE`.
Leave the `_env()` wrapper intact so environment variables still override.

### 2. Screening filters — ASK, never inherit
The file ships with a **working example set**. It is a starting point, not the user's rules, and a
user who never edits it screens against someone else's deal-breakers without knowing. Ask directly:

> Which industries are hard NOs for you? (Common ones: defense, gambling, crypto, adult, tobacco,
> predatory lending. Say "none" if you have none.)
> What is your work-location requirement — fully remote, hybrid, on-site, or no preference?
> Any political or values-based deal-breakers?
> Is private-equity ownership a concern for you?
> What is your salary floor? (0 means no filtering.)

Write the answers into `INDUSTRY_VETO`, `REMOTE_DISQUAL` / `REMOTE_CONFIRM`,
`POLITICS_DISQUAL` / `POLITICS_CLEAR`, `PE_FLAG` / `PE_CLEARED`, and `SALARY_FLOOR`.
⚠️ **Never leave `INDUSTRY_VETO` empty.** An empty screening list does not screen nothing loudly;
it passes everything silently. If the user has no industry vetoes, say so out loud and leave the
shipped defaults in place rather than blanking them.

### 3. Honesty guardrails — these stay EMPTY unless the user names something
`RETIRED` and `RETIRED_PATTERNS` encode figures the user has **corrected on their own résumé**.
Ask once:

> Is there a number or claim you used to put on your résumé that turned out to be wrong or
> overstated? The send gate will block it from ever going out again.

If they name one, add it. **If they do not, leave both lists empty.** Nobody else's retired-claims
list helps them, and a populated-by-default list would be a lie the gate then enforces.

For role-claim honesty, ask whether they are an engineer. If they are NOT, set `EMPLOYERS` to their
past employers so the gate catches "I built <employer>'s API" phrasing, and `SELF_BUILT` to things
they genuinely built themselves.

### 4. Verify before moving on
Run `python3 scripts/kit_config.py` and confirm it prints their real identity, then
`python3 scripts/doctor.py` and confirm section [1] is green.

## Step 3.6: Wire the enforcement hooks

The gates do not fire without this, and nothing errors to tell you — the enforcement layer is
simply absent. Copy the example into place:

```
cp .claude/settings.example.json .claude/settings.json
```

Confirm with `python3 scripts/doctor.py` — section [3] must read green.
...
>
> - `documents/PROFILE.md` - Your profile (the single source of truth the assistant loads)
...
> - `scripts/kit_config.py` - **Your identity and your deal-breakers** (you never had to edit it)
> - `.claude/settings.json` - Enforcement hooks wired on
>
> **Check it any time:** `python3 scripts/doctor.py` reports anything unconfigured or missing.
>
> **Try it out:**
...
- `documents/PROFILE.md` is the primary profile output and the single source of truth the assistant loads on demand; `CLAUDE.md` is a lean index that setup **never** edits (editing it would re-inflate it and cause `git pull` conflicts). The seven `01-07` skill files remain the detailed store that `/apply`, `/rank`, and `/interview` read, populated from the same answers.
