...
- **`scripts/check_preview.py`** — the `PreToolUse` hook. It **hard-blocks (exit 2)** any question carrying drafted outreach voice (kit_config `VOICE_MARKERS` / `VOICE_MARKER_PATTERNS` — two independent signals required) unless the ledger holds a real BUILD ruling. Fail-open on any error.
